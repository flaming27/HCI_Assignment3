...
Participants <- bind_rows(Participant1, Participant2, Participant7)
...
plot_grid({ 
  ...
  Participants %>% 
    ...
    stat_summary(fun.y = mean, geom = "line", position = pd) + 
    stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0, position = pd) + 
    ylab("Error") + #Mean(Time) and 95% CI") +
    expand_limits(y = 0)
  },
  nrow = 1)
```
...
```{r}
vTouch <- cbind(visionContr,       (visionContr * 0)) # building left side of matrix
vKnob  <- cbind((visionContr * 0), visionContr) # building right side of matrix (identical but shifted from vTouch)
