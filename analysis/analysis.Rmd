...
The multiple linear regression based on the AIC results provides an increased R-squared of 0.53. This is an improvement over both the "outlier-pruned" Model 1a (R-squared = 0.43) and the original linear Model 1 (R-squared 0.33). And unlike the outlier removal method, this preserves all data points. 
...
# Part IV Analysis - Block Effect

In exploratory analysis we noticed what appeared to be Block Effect to varying degrees for the conditions. Therefore we have built linear models (using AIC) to explore the effect of the presence (or lack of) the block variable. 

## Automatic model comparison: movement time
...
step_backward_result_block <- 
  MASS::stepAIC(lm_all_block, 
    direction = "backward", 
    trace = TRUE           # this shows the fit trace so we can track AIC (optional)
    )

step_backward_result_block$anova
anova_result_block <- anova(step_backward_result_block)
print(anova_result_block)
```

It should be noted that this model is different from the AIC model built for throughput. This means that MT and TP are affected differently, or that the presence of the TP acting on MT is different from MT acting on TP. 

#Multiple linear regression based on the AIC results (Block - MT)
...
library(lme4)
m_mt_b <- lm(MT.ms. ~ Condition + Block + TP.bps. + TRE + MDC + ODC + ME, data= Participants) 
m_mt_b.red <- lm(MT.ms. ~ Condition + TP.bps. + TRE + MDC + ODC + ME, data= Participants) 
anova(m_mt_b, m_mt_b.red, test="LRT")

```

From this we see that there does appear to be a block effect on on MT, with p < 0.056. 


#Multiple linear regression based on the AIC results (Block - Throughput)

We already have the AIC for throughput from earlier, so we just plug it in and compare. 

```{r}
Participants$Condition <- as.factor(Participants$Condition)

m_tb_b <- lm(TP.bps. ~ Condition + Block + MT.ms. + TRE + MDC + ODC + ME, data= Participants)
m_tb_b.red <- lm(TP.bps. ~ Condition + MT.ms. + TRE + MDC + ODC + ME, data= Participants)
anova(m_tb_b, m_tb_b.red, test="LRT")

```

Here we see that -- again -- there appears to be a block effect, with p < 0.084. 

We can even try improving this further by removing presumed outliers that are impacting the results. 

```{r}
Participants$Condition <- as.factor(Participants$Condition)

m_tb_b <- lm(TP.bps. ~ Condition + Block + MT.ms. + TRE + MDC + ODC + ME, data= Participants[Participants$TP.bps.>0.55,])
m_tb_b.red <- lm(TP.bps. ~ Condition + MT.ms. + TRE + MDC + ODC + ME, data= Participants[Participants$TP.bps.>0.55,])
anova(m_tb_b, m_tb_b.red, test="LRT")

```

Removing the presumed erroneous outliers decreases the p further, to 0.03. The SoS converges slightly closer to 0. 

## Automatic model comparison: accuracy (MDC) 
...
step_backward_result_block2 <- 
  MASS::stepAIC(lm_all_block2, 
    direction = "backward", 
    trace = TRUE           # this shows the fit trace so we can track AIC (optional)
    )

step_backward_result_block2$anova
anova_result_block2 <- anova(step_backward_result_block2)
print(anova_result_block2)
```

In the AIC calculation for accuracy (MDC), we see that Block has been eliminated in the final step. Given the change in MDC due to Block (2.21 -> 2.19), we agree with the AIC that block does not appear to have had an effect on the accuracy measure. 
