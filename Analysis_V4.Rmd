...
With the added model_1a, we see that both hypotheses have estimates closer to 0. However, for the purposes of this study we will use the results from the full test group (all sequences from all participants - model_1), as that was mentioned in our protocols as a goal and the results (at least for TP) are similar. 
...
Based on exploratory analysis we saw that three of the accuracy measures (MDC, ODC, and error %) demonstrated similar scaled results for each condition, with ME deviating from the rest. We then looked at model testing for each. Based on model testing we found that MDC seemed to provide the best model fit of MDC, ODC, ME, and error %. For ease of reading we have left just the MDC. 
```{r}
...
Normal Q-Q shows the "best fit" for residuals with an MDC linear model when compared with other accuracy measures. Therefore we are proceeding with this. 
...
Although taken on its own the two-handed does appear to be more accurate than sway, this was not part of this hypothesis.
NOT ACCEPTED
